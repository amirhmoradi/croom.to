"""
OS Support and Rollback for PiMeet.
...
class RollbackService:
    """
    Version rollback service for PiMeet.
    ...
    def __init__(
        self,
        snapshot_dir: str = "/var/lib/pimeet/snapshots",
        max_snapshots: int = 5,
    ):
    ...
    async def create_snapshot(
        ...
        # Get current version
        version = packages.get("pimeet", "unknown")
    ...
    async def _get_installed_packages(self) -> Dict[str, str]:
        """Get installed PiMeet package versions."""
        packages = {}
    ...
    async def _backup_config(self, snapshot_id: str) -> str:
        ...
        config_dir = Path("/etc/pimeet")
        if config_dir.exists():
            shutil.copytree(config_dir, backup_dir / "pimeet", dirs_exist_ok=True)
    ...
    async def _restore_config(self, backup_path: str) -> None:
        ...
        pimeet_config = backup_dir / "pimeet"
        if pimeet_config.exists():
            shutil.copytree(pimeet_config, "/etc/pimeet", dirs_exist_ok=True)
...
class LegacyMigration:
    """
    Migration tool for legacy PiMeet image-based installations.
    ...
    LEGACY_PATHS = {
        "config": "/home/pi/.pimeet",
        "app": "/opt/pimeet",
        "service": "/etc/systemd/system/pimeet.service",
        "data": "/var/lib/pimeet",
    }
    ...
    async def detect_legacy(self) -> bool:
        ...
        if self._legacy_detected:
            # Try to get version
            version_file = Path("/home/pi/.pimeet/version")
    ...
    async def _create_backup(self) -> None:
        ...
        backup_dir = Path(f"/var/backups/pimeet-legacy-{datetime.now().strftime('%Y%m%d-%H%M%S')}")
        backup_dir.mkdir(parents=True, exist_ok=True)
    ...
    async def _stop_legacy_services(self) -> None:
        ...
        services = ["pimeet", "pimeet-ui", "pimeet-browser"]
    ...
    async def _install_packages(self) -> None:
        """Install PiMeet packages."""
        subprocess.run(["apt", "update"], check=True)
        subprocess.run(["apt", "install", "-y", "pimeet"], check=True)
    ...
    async def _cleanup_legacy(self) -> None:
        ...
        for service_file in Path("/etc/systemd/system").glob("pimeet*.service"):
            service_file.unlink()
    ...
    async def _start_services(self) -> None:
        """Start new PiMeet services."""
        subprocess.run(["systemctl", "enable", "pimeet"], check=True)
        subprocess.run(["systemctl", "start", "pimeet"], check=True)
