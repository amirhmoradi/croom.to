"""
Remote operations service for PiMeet.
...
class DiagnosticRunner:
    ...
    async def _test_disk_write(self) -> float:
        ...
        try:
            test_file = Path("/tmp/pimeet_disk_test")
    ...
    async def _test_services(self) -> DiagnosticResult:
        """Test PiMeet services."""
        start = datetime.utcnow()
        try:
            services = ["pimeet-agent", "pimeet-browser"]
            status = {}
...
class LogCollector:
    ...
    async def collect(
        ...
        # PiMeet logs
        pimeet_logs = await self._collect_pimeet_logs(since, max_lines)
        if pimeet_logs:
            logs["pimeet"] = pimeet_logs
    ...
    async def _collect_pimeet_logs(
        self,
        since: Optional[datetime],
        max_lines: int,
    ) -> Dict[str, str]:
        """Collect PiMeet application logs."""
        logs = {}
        pimeet_log_dir = Path("/var/log/pimeet")

        if not pimeet_log_dir.exists():
            return logs

        for log_file in pimeet_log_dir.glob("*.log"):
            try:
                lines = []
    ...
    async def create_support_bundle(
        ...
        bundle_name = f"pimeet_support_{timestamp}"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_paths = [
                Path("/etc/pimeet/config.json"),
                Path("/etc/pimeet/device.json"),
            ]
...
class UpdateManager:
    ...
    async def check_updates(self) -> Dict[str, Any]:
        ...
        # Check PiMeet updates
        if self._update_url:
            pimeet_update = await self._check_pimeet_update()
            if pimeet_update:
                result["updates_available"] = True
                result["updates"].append(pimeet_update)

        return result

    async def _get_current_version(self) -> str:
        """Get current PiMeet version."""
    ...
    async def _check_pimeet_update(self) -> Optional[Dict[str, Any]]:
        """Check for PiMeet application update."""
        ...
        try:
            ...
            req = Request(
                ...
                headers={"User-Agent": "PiMeet-Update/1.0"},
            )
            ...
            logger.error(f"Failed to check PiMeet updates: {e}")
    ...
    async def apply_updates(
        ...
        Args:
            update_type: Type of updates ("apt", "pimeet", "all")
        ...
        if update_type in ("pimeet", "all") and self._update_url:
            if on_progress:
                on_progress("Updating PiMeet...", 50)

            pimeet_result = await self._apply_pimeet_update()
            if pimeet_result["success"]:
                result["updated"].append(f"pimeet-{pimeet_result.get('version', 'unknown')}")
            elif pimeet_result.get("error"):
                result["errors"].append(pimeet_result["error"])
    ...
    async def _apply_pimeet_update(self) -> Dict[str, Any]:
        """Apply PiMeet application update."""
...
class RemoteOperationsService:
    ...
    def __init__(
        ...
        Args:
            ...
            update_url: URL for PiMeet updates
            allowed_commands: List of allowed command prefixes for execute_command
        """
        self._storage_path = storage_path or Path("/var/lib/pimeet/operations")
    ...
    async def _do_restart_service(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Restart a service."""
        service = params.get("service", "pimeet-agent")

        # Validate service name (only allow pimeet services)
        if not service.startswith("pimeet"):
    ...
    async def _do_update_config(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config_path = Path("/etc/pimeet/config.json")
...
def create_remote_ops_service(config: Dict[str, Any]) -> RemoteOperationsService:
    ...
    return RemoteOperationsService(
        storage_path=Path(config.get("storage_path", "/var/lib/pimeet/operations")),
