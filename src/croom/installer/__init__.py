...
class InstallationConfig:
    ...
    target_path: str = "/opt/croom"
    config_path: str = "/etc/croom"
    data_path: str = "/var/lib/croom"
...
class Installer:
    ...
    async def install(self) -> bool:
        ...
        logger.info(f"Starting Croom installation v{self.VERSION}")
    ...
    async def _install_core(self) -> bool:
        ...
        dst_path = target / "lib" / "python" / "croom"
    ...
    async def _install_ui(self) -> bool:
        ...
        # Copy QML files
        src_ui = Path(__file__).parent.parent.parent / "croom-ui"
        if src_ui.exists():
    ...
    async def _install_dashboard(self) -> bool:
        ...
        src_dashboard = Path(__file__).parent.parent.parent / "croom-dashboard"
    ...
    async def _install_dashboard_frontend(self) -> bool:
        ...
        # Build frontend
        src_frontend = Path(__file__).parent.parent.parent / "croom-dashboard-frontend"
...
Description=Croom Agent Service
...
ExecStart={sys.executable} -m croom.core.agent
Restart=always
RestartSec=10
...
Description=Croom Dashboard Service
After=network.target croom-agent.service
...
"""
        ...
        (services_path / "croom-agent.service").write_text(agent_service)
        (services_path / "croom-dashboard.service").write_text(dashboard_service)

        # Create croom user if not exists
        try:
            subprocess.run(
                ["useradd", "-r", "-s", "/bin/false", "croom"],
                capture_output=True,
            )
        except Exception:
            pass
        ...
        # Set ownership
        subprocess.run(
            ["chown", "-R", "croom:croom", self._config.target_path],
            capture_output=True,
        )
        subprocess.run(
            ["chown", "-R", "croom:croom", self._config.data_path],
            capture_output=True,
        )
        ...
        if self._config.enable_autostart:
            subprocess.run(
                ["systemctl", "enable", "croom-agent"],
                capture_output=True,
            )
            subprocess.run(
                ["systemctl", "enable", "croom-dashboard"],
                capture_output=True,
            )
...
Exec={sys.executable} -m croom.ui.main
Icon={self._config.target_path}/assets/icon.png
Terminal=false
Type=Application
Categories=Office;Network;
"""
            desktop_path = Path("/usr/share/applications/croom.desktop")
...
class Uninstaller:
    ...
    async def uninstall(self) -> bool:
        ...
        logger.info("Starting Croom uninstallation")

        try:
            # Stop services
            subprocess.run(
                ["systemctl", "stop", "croom-agent"],
                capture_output=True,
            )
            subprocess.run(
                ["systemctl", "stop", "croom-dashboard"],
                capture_output=True,
            )

            # Disable services
            subprocess.run(
                ["systemctl", "disable", "croom-agent"],
                capture_output=True,
            )
            subprocess.run(
                ["systemctl", "disable", "croom-dashboard"],
                capture_output=True,
            )

            # Remove service files
            Path("/etc/systemd/system/croom-agent.service").unlink(missing_ok=True)
            Path("/etc/systemd/system/croom-dashboard.service").unlink(missing_ok=True)
            subprocess.run(["systemctl", "daemon-reload"], capture_output=True)

            # Remove installation
            shutil.rmtree("/opt/croom", ignore_errors=True)

            # Remove config if not keeping
            if not self._keep_config:
                shutil.rmtree("/etc/croom", ignore_errors=True)

            # Remove data if not keeping
            if not self._keep_data:
                shutil.rmtree("/var/lib/croom", ignore_errors=True)

            # Remove logs
            shutil.rmtree("/var/log/croom", ignore_errors=True)

            # Remove desktop entry
            Path("/usr/share/applications/croom.desktop").unlink(missing_ok=True)
...
from croom.installer.os_support import (
    OSType,
    OSRelease,
    OSInfo,
    VersionSnapshot,
