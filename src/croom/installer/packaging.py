"""
APT Package Repository and Update System for Croom.
...
class PackageType(Enum):
    ...
    UI = "croom-ui"
    AI = "croom-ai"
    DASHBOARD = "croom-dashboard"
    MEETING = "croom-meeting"
    FULL = "croom"  # Meta-package
...
class PackageInfo:
    ...
    description: str
    maintainer: str = "Croom Team <team@croom.io>"
    homepage: str = "https://croom.io"
    section: str = "misc"
...
PACKAGE_DEFINITIONS = {
    PackageType.CORE: PackageInfo(
        ...
        description="Croom Core - Base conference room system",
        depends=COMMON_DEPENDS + [
            "chromium-browser",
        ],
        recommends=["croom-ui", "croom-meeting"],
    ),
    PackageType.UI: PackageInfo(
        ...
        distribution=Distribution.BOOKWORM,
        description="Croom Touch UI - QML-based touch interface",
        depends=[
            "croom-core",
            "qml-module-qtquick2",
    ...
    PackageType.AI: PackageInfo(
        ...
        architecture=Architecture.ARM64,
        distribution=Distribution.BOOKWORM,
        description="Croom AI - Edge AI features for video conferencing",
        depends=[
            "croom-core",
            "python3-numpy",
            "python3-opencv",
    ...
    PackageType.DASHBOARD: PackageInfo(
        ...
        architecture=Architecture.ALL,
        distribution=Distribution.BOOKWORM,
        description="Croom Dashboard Client - Remote management agent",
        depends=["croom-core"],
    ),
    PackageType.MEETING: PackageInfo(
        ...
        architecture=Architecture.ALL,
        distribution=Distribution.BOOKWORM,
        description="Croom Meeting Providers - Google Meet, Teams, Zoom, Webex",
        depends=[
            "croom-core",
            "chromium-browser",
        ],
    ...
    PackageType.FULL: PackageInfo(
        ...
        architecture=Architecture.ALL,
        distribution=Distribution.BOOKWORM,
        description="Croom - Complete conference room system",
        depends=[
            "croom-core",
            "croom-ui",
            "croom-meeting",
...
PACKAGE_DEFINITIONS_AMD64 = {
    PackageType.CORE: PackageInfo(
        ...
        distribution=Distribution.BOOKWORM,
        description="Croom Core for x86_64 - Base conference room system",
        depends=COMMON_DEPENDS + AMD64_DEPENDS,
        recommends=["croom-meeting"],
    ),
    PackageType.AI: PackageInfo(
        name="croom-ai",
        version="1.0.0",
        architecture=Architecture.AMD64,
        distribution=Distribution.BOOKWORM,
        description="Croom AI for x86_64 - GPU-accelerated AI features",
        depends=[
            "croom-core",
        ] + AMD64_AI_DEPENDS,
    ...
    PackageType.FULL: PackageInfo(
        ...
        architecture=Architecture.AMD64,
        distribution=Distribution.BOOKWORM,
        description="Croom for x86_64 - Complete conference room system",
        depends=[
            "croom-core",
            "croom-meeting",
        ],
...
PACKAGE_DEFINITIONS_UBUNTU = {
    PackageType.CORE: PackageInfo(
        ...
        description="Croom Core for Ubuntu - Base conference room system",
        depends=COMMON_DEPENDS + [
    ...
    PackageType.AI: PackageInfo(
        ...
        architecture=Architecture.AMD64,
        distribution=Distribution.JAMMY,
        description="Croom AI for Ubuntu - GPU-accelerated AI features",
        depends=[
            "croom-core",
            "python3-numpy",
            "python3-opencv",
...
class DebianPackageBuilder:
    """
    Builds Debian packages for Croom components.
    """
    ...
    async def _add_core_files(self, build_dir: Path) -> None:
        ...
        # Python package
        lib_dir = build_dir / "usr/lib/python3/dist-packages/croom"
        lib_dir.mkdir(parents=True)

        # Copy core modules
        core_modules = ["core", "services", "platform", "config"]
        for module in core_modules:
            src = self._source_dir / "src/croom" / module
            if src.exists():
...
Description=Croom Conference Room System
After=network.target

[Service]
Type=simple
User=croom
ExecStart=/usr/bin/croom
Restart=always
RestartSec=5
...
"""
        (systemd_dir / "croom.service").write_text(service_content)
...
exec python3 -m croom "$@"
"""
        wrapper_file = bin_dir / "croom"
        wrapper_file.write_text(wrapper_content)
        wrapper_file.chmod(0o755)

        # Configuration
        etc_dir = build_dir / "etc/croom"
        etc_dir.mkdir(parents=True)

    async def _add_ui_files(self, build_dir: Path) -> None:
        """Add UI package files."""
        # QML files
        share_dir = build_dir / "usr/share/croom/qml"
        share_dir.mkdir(parents=True)

        qml_src = self._source_dir / "src/croom-ui/qml"
        if qml_src.exists():
            shutil.copytree(qml_src, share_dir, dirs_exist_ok=True)

        # Python UI module
        lib_dir = build_dir / "usr/lib/python3/dist-packages/croom/ui"
        lib_dir.mkdir(parents=True)

        ui_src = self._source_dir / "src/croom/ui"
        if ui_src.exists():
            shutil.copytree(ui_src, lib_dir, dirs_exist_ok=True)

    async def _add_ai_files(self, build_dir: Path) -> None:
        """Add AI package files."""
        lib_dir = build_dir / "usr/lib/python3/dist-packages/croom/ai"
        lib_dir.mkdir(parents=True)

        ai_src = self._source_dir / "src/croom/ai"
        if ai_src.exists():
            shutil.copytree(ai_src, lib_dir, dirs_exist_ok=True)

        # Models directory
        models_dir = build_dir / "usr/share/croom/models"
        models_dir.mkdir(parents=True)

    async def _add_meeting_files(self, build_dir: Path) -> None:
        """Add meeting provider files."""
        lib_dir = build_dir / "usr/lib/python3/dist-packages/croom/meeting"
        lib_dir.mkdir(parents=True)

        meeting_src = self._source_dir / "src/croom/meeting"
    ...
    async def _add_dashboard_files(self, build_dir: Path) -> None:
        ...
        lib_dir = build_dir / "usr/lib/python3/dist-packages/croom/dashboard"
        lib_dir.mkdir(parents=True)

        dashboard_src = self._source_dir / "src/croom/dashboard"
...
case "$1" in
    configure)
        # Create croom user if not exists
        if ! id -u croom > /dev/null 2>&1; then
            useradd -r -s /bin/false -d /var/lib/croom croom
        fi

        # Create directories
        mkdir -p /var/lib/croom
        mkdir -p /var/log/croom
        mkdir -p /etc/croom

        # Set permissions
        chown -R croom:croom /var/lib/croom
        chown -R croom:croom /var/log/croom
        chown -R root:croom /etc/croom
        chmod 750 /etc/croom
...
"""
        if package_type == PackageType.CORE:
            script += """
        # Stop service
        systemctl stop croom.service || true
        systemctl disable croom.service || true
"""
...
class APTRepository:
    """
    APT repository management for Croom packages.
    """
...
class APTUpdateService:
    """
    APT-based update service for Croom.
    """

    def __init__(
        self,
        repo_url: str = "https://apt.croom.io",
...
"""
            source_file = Path("/etc/apt/sources.list.d/croom.list")
            ...
            subprocess.run([
                "curl", "-fsSL", key_url, "|",
                "gpg", "--dearmor", "-o", "/usr/share/keyrings/croom-archive-keyring.gpg",
            ], shell=True, check=True)

            # Update sources to use keyring
            source_content = f"""deb [signed-by=/usr/share/keyrings/croom-archive-keyring.gpg] {self._repo_url} {self._distribution.value} main
"""
            source_file.write_text(source_content)
    ...
    async def check_updates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check for upgradable croom packages
            result = subprocess.run(
                ["apt", "list", "--upgradable"],
                capture_output=True,
                text=True,
    ...
    async def get_installed_version(self, package: str = "croom") -> Optional[str]:
